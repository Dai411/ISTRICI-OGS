        program gridinterpolation    
        implicit real (a-g,p-z)
        dimension nindice(10000,2),xmatvet(10000,2),diff(10000)
        dimension vg1(10000,10000)
        dimension zbot1(10000),ztop(10000),zbot(10000),vg(10000,10000)
        ...
        if(nx.gt.10000) then
         print*, 
     $'please, change the matrix dimention in gridinterpolation.f'
         stop
        endif
...
c            write(79,*)
c     $    i,j,x,z,vel,xmatvet(i,2),zbot(i),zbot1(i),ztop(i)
            if(ztop(i).gt.zbot1(i)) 
     $ print*, 
     $ 'Attention: the zbottom is now above the ztop at the postion:',x
c            zbot1(i)=ztop(i)-dz
           endif
          endif
          ...
          if(z.le.zbot1(i).and.z.ge.ztop(i)) then
           vel=vel+xmatvet(i,2)
           vel1=vel
c           write(78,*) x,z,i,j,vel2,vel,xmatvet(i,2)
           nindice(i,1)=j
          endif
          vg(i,j)=vel
         enddo
...
c
 ...
 10    continue 
c	print*, 'do you have deeper horizon to be updated? (yes=1)'
c        read*, nopt
c        if(nopt.ne.1) goto 79

c        print*, 'give me the filename of the horizon'
c        read(*,'(a30)') file3
c        print*, 'give me the new filename'
c        read(*,'(a30)') file4

c        open(30,file=file3)
c        open(31,file=file4)
c        do i=1,nx
c         read(30,*) x,y
c         y1=y+diff(i)
c         write(31,*) x,y1
